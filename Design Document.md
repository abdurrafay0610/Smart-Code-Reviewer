...
An AI pre-review assistant for C++ that runs *before* a human opens a pull request. It reviews code on three axes — **readability, structure, and maintainability** — and additionally catches **architectural drift**: code that is locally clean but doesn't fit the established shape of the codebase.
...
> **Language:** C++. The toolchain — clang-tidy, cppcheck, clang-format, lizard — is standard C++ static analysis, and the scope is deliberately narrowed to one language so the deterministic evaluators are easy to stand up (a one-line `apt-get` each in a Colab or Streamlit container).
...
flowchart TD
    ...
    subgraph PhaseB["Phase B — Review a Change (on branch selection)"]
        ...
        Changed --> Tools["Run deterministic tools on FULL changed files<br/>clang-tidy · cppcheck · clang-format · lizard"]
...
|---|---|---|---|
| Identifier naming | clang-tidy `readability-identifier-naming` | Unclear / inconsistent names vs a chosen convention | Any violation (configurable) |
| Magic numbers | clang-tidy `readability-magic-numbers` | Unnamed literals that should be constants | Any non-trivial literal |
| Missing braces | clang-tidy `readability-braces-around-statements` | Single-statement `if`/`for` without braces | Any occurrence |
| Redundant control flow | clang-tidy `readability-else-after-return`, `-simplify-boolean-expr` | `else` after `return`, convoluted booleans | Any occurrence |
| Formatting drift | clang-format (diff) | Indentation, spacing, line length, brace style | % of lines reformatted — higher is worse |

### Structure

| Signal | Tool | What it flags | Rough threshold |
|---|---|---|---|
| Cyclomatic complexity (CCN) | lizard | Independent paths through a function | >10 smell, >15 concerning, >20 refactor (lizard warns at 15 by default) |
| Function length (NLOC) | lizard | Lines per function → doing too much | >50 look, >100 bad |
| Parameter count | lizard | Long argument lists → weak cohesion | >4 smell |
| Cognitive complexity / nesting | clang-tidy `readability-function-cognitive-complexity` | Deeply nested, hard-to-track logic | >15–25 (configurable) |
| Decomposition | lizard (summary) | God-functions vs sensible breakdown | Qualitative |

### Maintainability

| Signal | Tool | What it flags | Rough threshold |
|---|---|---|---|
| Static-analysis warnings | cppcheck | Bugs, dead code, unused vars, resource leaks | Density (warnings per LOC) |
| Outdated idioms | clang-tidy `modernize-*` | Raw pointers, C-style casts, no `auto` / range-`for` | Any occurrence |
| Code duplication | lizard duplicate extension (or a simple token check) | Copy-pasted blocks → change amplification | Duplicated blocks present |
| Composite index (concept) | Maintainability Index — CCN + LOC + Halstead volume | Single 0–100 "how painful to maintain" score | 0–100, higher better; C++ tooling here is thin, so approximate from its inputs |

**Routing note:** function length lives only under Structure, even though clang-tidy's `readability-function-size` could also report it. Leaving both on would surface the same number in two tabs and manufacture the overlap the design is trying to avoid. One signal, one home.

---

## 6. The three review agents

The agents are **presentation specialists**, each interpreting its own slice of the evidence bundle. Because the evidence is partitioned upfront (§5), the agents cannot repeat each other across tabs.

- **Readability agent** — takes the naming, magic-number, brace, control-flow, and formatting findings and explains them in human terms, plus makes the qualitative calls tools can't: is this name actually *good*, not just conformant.
...
2. **Per-file language + imports** — extension maps to language; `#include`/import extraction via regex or tree-sitter. Deterministic. Fed as ground truth into step 3.
